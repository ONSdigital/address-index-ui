...
def get_current_selected_endpoint(endpoints, called_from):
  ...
  for endpoint in endpoints:
    if endpoint.get('page_name') == called_from:
      return endpoint
    # Now check for sub-nav possibilities
    if endpoint.get('sub_nav_endpoints'):
      for sub_endpoint in endpoint.get('sub_nav_endpoints'):
        if sub_endpoint.get('page_name') == called_from:
          return sub_endpoint
...
def get_endpoints(called_from=''):
  ...
  # Create the URL for each endpoint
  for endpoint in endpoints:
    if not endpoint.get('url'):
      endpoint['url'] = url_for(endpoint.get('page_name'))

    if endpoint.get('sub_nav_endpoints'):
      for sub_endpoint in endpoint.get('sub_nav_endpoints'):
        if not sub_endpoint.get('url'):
          sub_endpoint['url'] = url_for(sub_endpoint.get('page_name'))
