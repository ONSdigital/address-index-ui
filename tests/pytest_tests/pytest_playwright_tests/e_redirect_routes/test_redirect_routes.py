...
def test_page_redirects(page: Page, endpoint_info, current_role, login_and_goto_url):
  ...
  print(f'Role: {user_role}, Endpoint: {endpoint_info.get("url")} -> {endpoint_info.get("redirect_url")} -> {endpoint_info.get("redirect_visible_items")} ')
  ...
  if redirect_url != None:
    ...
    for expected_text in expected_texts:
      expect(page.locator('body').filter(has_text=expected_text)).to_be_visible()
